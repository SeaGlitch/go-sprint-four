...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedMINS := meanSpeed(action, duration) * kmhInMsec // скорость в м/с
	walkCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMINS, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimCalories := (speed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimCalories
}
